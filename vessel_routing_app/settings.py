"""
Django settings for vessel_routing_app project.
...
SECRET_KEY = 'b6a-tyefd*s_+h*8qv*ei_5zr3x2gfp6qjj^w)06jq_^kmca$6'
...
ROOT_URLCONF = 'vessel_routing_app.urls'
...
WSGI_APPLICATION = 'vessel_routing_app.wsgi.application'
...
# Using the collectstatic command, Django looks for all static files in your apps and collects them wherever you
# told it to, i.e. the STATIC_ROOT. In our case, we are telling Django that when we run python manage.py collectstatic,
